...
`S_n` such that `sigma in X` implies `sigma^2 not in X`.  The following
recurrence computes `F_n` using one state for each partition of `n`.
...
`q(mu)` be the cycle type obtained by squaring a permutation of type `mu`:
an odd `j`-cycle remains a `j`-cycle, while an even `2j`-cycle becomes two
`j`-cycles.
...
square roots of type `mu`.  Squaring maps the conjugacy class of `mu`
equivariantly onto that of `lambda`, so every fiber has cardinality
`(n!/z_mu)/(n!/z_lambda)`.

## Reverse-tree recurrence

For a type `lambda` containing an even part, let `E_lambda` and `I_lambda`
be the largest sizes of an independent set in the reverse tree below one
...
of a cycle length in `lambda`, every type on the right side has valuation
`v(lambda)+1`.  Descending valuation therefore determines all nonperiodic
states.

For a type whose parts are all odd, equations (2) instead give the weights of
the trees attached to a cycle vertex, with the unique same-type predecessor
omitted.
...
Take `L_lambda=1` when `o_lambda=1`.  The conjugacy class splits into
`N_lambda` directed squaring cycles of length `L_lambda`.  Put
`g_lambda=max(0,I_lambda-E_lambda)`.  When `L_lambda>1`, each component
contributes
...
When `L_lambda=1`, the loop forbids its root, and the contribution is just
`E_lambda`.  Summing (4), multiplied by `N_lambda`, over all all-odd types
gives `F_n`.  Equations (1)--(4) constitute an exact finite recurrence for
every `n`.
...
The recurrence was checked through `n=50`; this is a verification horizon,
not a limit of the theorem. An optimized C++ program builds
the complete squaring functional graph through `S_11` and applies the exact
...
submission target is 10,000 terms.
